import type { UiDayData } from ".calculations/uiDataProfile";
 const dummyWeatherDataApril: UiDayData[] = [
...
// Would it be possible to wrap the pool.queries in service function to avoid importing 
// the pool to the data manager? 
*/

// service
export async function getPriceProfile(date_from: string, date_to: string) {
    const result = await pool.query(...);
    return result.rows;
}

// route
app.get('/api/price-profile/range', async (req, res) => {
    const rows = await getPriceProfile(...);
    res.json(rows);
});

const priceData = await getPriceProfile(date_from: string, date_to: string)
const 

export const getUiDayData(periodStart: number, periodEnd: number): UiDayData[] => {
    //*** creates UiDayData***
    return dummyWeatherDataApril;  // (list of UiDayData objects)
}
