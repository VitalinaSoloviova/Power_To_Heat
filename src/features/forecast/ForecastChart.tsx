...
const ForecastChart = ({ data }: ForecastChartProps) => {
  ...
  const xLabels = data.map((h) =>
    h.datetime.toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })
  );
  const minTemp = data.map((h) => h.weather.minTemp);
  const maxTemp = data.map((h) => h.weather.maxTemp);
  const energyDemand = data.map((h) => h.energyDemand);
        ...
        <Box sx={{ display: 'flex', gap: 1.5 }}>
          {[
            ...
            <Box key={x.l} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Box sx={{ width: 14, height: 0, borderTop: `2px ${x.dashed ? 'dashed' : 'solid'} ${x.c}` }} />
